export function applyCarlottaLogic({
    ...
    if (tab === 'forteCircuit' || tab === 'resonanceLiberation') {
        ...
        if (name.includes("closing remark")) {
            skillMeta.multiplier = 794.2/100;
        } else {
            skillMeta.multiplier = 1032.18/100;
        }
    }
    ...
    if (isToggleActive(1) && isActiveSequence(1)) {
        if (!mergedBuffs.__seq1) {
            mergedBuffs.critRate = (mergedBuffs.critRate ?? 0) + 12.5;
    ...
    if (isActiveSequence(2) && tab === 'resonanceLiberation' && name === 'fatal finale dmg') {
        skillMeta.multiplier *= 2.26;
    }

    if (isActiveSequence(3) && tab === 'resonanceSkill') {
        skillMeta.multiplier *= 1.93;
    }
    ...
    if (isActiveSequence(5) && tab === 'forteCircuit' && name === 'imminent oblivion dmg') {
        skillMeta.multiplier *= 1.47;
    }

    if (isActiveSequence(6) && tab === 'resonanceLiberation' && name === 'death knell dmg') {
        skillMeta.multiplier *= 2.866;
    }
...
export function carlottaBuffsLogic({
